...
void editorUpdateSyntax(erow* row) {
    ...
    while (i < row->rsize) {
        ...
        if (scs_len && !in_string) {
            if (!strncmp(&row->hl[i], scs, scs_len)) {
...
void editorDrawRows(struct abuf* ab) {
    ...
    for (y = 0; y < E.screenrows; y++) {
        ...
        abAppend(ab, "\x1b[K", 3); // erases part of the line (right to the cursor)    
        abAppend(ab, "\r\n", 2);
    }
}

void editorDrawStatusBar(struct abuf* ab) {
    abAppend(ab, "\x1b[7m", 4); // invert colors (black text and white background)
...
void editorRefreshScreen() {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (E.cy - E.rowoff) + 1, (E.rx - E.coloff) + 1); // terminal use 1-index values
...
void initEditor() {
    ...
    if (getWindowSize(&E.screenrows, &E.screencols) == -1)
        die("getWindowSize");
    E.screenrows -= 2; // preserve the last 2 for status bar and message
}
